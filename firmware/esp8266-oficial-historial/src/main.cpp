/* ============================================================
   ...
   así el "Historial" del tablero por fin se llena para todos.

   Lee UN sensor a la vez desde A0 (agua o aire), según cuál cable
   de datos esté conectado. Un jumper en D7 le dice al código cuál.
   ...
   >> ¿Qué cambia respecto a la versión base? Solo el bloque marcado
      "LA MODIFICACIÓN: HISTORIAL" dentro de loop(). Todo lo demás
      es idéntico. El oficial hacía solo set() (pisa el dato); aquí
      además hacemos push() a /historial (acumula sin pisar).
   ============================================================ */
...
void loop() {
  if (millis() - ultimaLectura >= INTERVALO) {
    ...
    float valor;          // valor que se publica: ppm si agua, crudo si aire
    const char* tipo;

    if (modoAgua) {
      valor = TDS_MIN_PPM + (lecturaCruda - TDS_MIN_LECTURA) *
              (TDS_MAX_PPM - TDS_MIN_PPM) / (TDS_MAX_LECTURA - TDS_MIN_LECTURA);
      tipo = "agua";
      Firebase.setFloat(fbData, "/sensor/agua", valor);
      Serial.print("[" + timestamp + "] MODO AGUA | ppm: ");
      Serial.println(valor);
    } else {
      valor = lecturaCruda;
      tipo = "aire";
      Firebase.setFloat(fbData, "/sensor/aire", valor);
    ...
    // ===== LA MODIFICACIÓN: HISTORIAL con push (acumula en vez de pisar) =====
    // El firmware oficial solo hace set() en /sensor -> cada lectura pisa la
    // anterior y no queda historial. Aquí, además, hacemos push() a /historial:
    // cada lectura crea una entrada NUEVA con clave única (ordenada por tiempo),
    // con la forma que el dashboard del colectivo espera leer:
    //   { "tipo": "agua"|"aire", "valor": <numero>, "timestamp": "YYYY-MM-DD HH:MM:SS" }
    FirebaseJson entrada;
    entrada.set("tipo", tipo);
    entrada.set("valor", valor);
    entrada.set("timestamp", timestamp);
    if (!Firebase.pushJSON(fbData, "/historial", entrada)) {
      Serial.println("   [Firebase][X] No se pudo guardar /historial: " + fbData.errorReason());
    } else {
      Serial.println("   [Firebase] historial guardado (push).");
    }
  }
}
